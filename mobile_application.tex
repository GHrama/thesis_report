...
The following sections will explain the integral parts of the server and client of the mobile application. A summary of the architecture is shown
...
Users can login onto the FairDataShare Portal from their computer or the mobile application. Once the user is authenticated, the user requests
...
The first time the application opens, the alarm is set to ring in exactly 24 hours, but the situation changes when the phone is switched off.
...
As mentioned before, the current time of executing the alarm is saved in case the phone is rebooted or switched off. After that, the sensor data which is saved locally
...
Kinvey\footnote{\url{http://kinvey.com/}} is a mobile backend as a service which provides a platform for mobile phones to link applications to a backend cloud storage. For the purpose of this application, the backend has been used to store data and for some business logic implementations in javascript.
